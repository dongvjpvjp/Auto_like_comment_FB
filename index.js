...
const login_link = "https://mbasic.facebook.com";

/* ^^^^^^^^^^^^[CẤU HÌNH HTML]^^^^^^^^^^^^^^ */

/* vvvvvvvvvvvv[KHỞI TẠO TRÌNH DUYỆT]vvvvvvvvvvvvvv */
const path = require('path');
const geckoDriverPath = path.join(__dirname, "chromedriver.exe"); // or wherever you've your geckodriver
chrome.setDefaultService(new chrome.ServiceBuilder(geckoDriverPath).build());
// chrome.setDefaultService(new chrome.ServiceBuilder(chrome.path).build());
const driver = new webdriver.Builder().withCapabilities(webdriver.Capabilities.chrome()).build();
/* ^^^^^^^^^^^^^[KHỞI TẠO TRÌNH DUYỆT]^^^^^^^^^^^^^ */

(async function example() {
  // Khởi tạo quá trình bất đồng bộ
	try {
      ...
      process.stdin.on("keypress", async (str, key) => {
        if (str == "Y") {
          ...
          console.log("\n Mở kết nối tài khoản facebook");
              ...
              console.log("Skip Interval Function");
						else if (now_date < config.target_date) {
...
async function like(id = null, type) {
	try {
    if (id) await visit(`https://mbasic.facebook.com/${id}`);
    setTimeout(async () => {
      const like_button = await findbyCSS(
        "a[href*='/a/like.php']:first-of-type"
      );
		await like_button.click();
      console.log("Truy cập và like bài viết", id);
      if (id) await getCoin(type, id);
    }, id ? 2000 : 0);
  } catch (err) {
    console.log("err", err);
		// await like(id);
...
async function cmt(id = null, msg) {
	try {
    if (id) await visit(`https://mbasic.facebook.com/${id}`);
    setTimeout(async() => {
		...
		if (id) await getCoin("comment", id);
	}, id ? 2000 : 0);
	} catch (err) {
    console.log("err", err);
	}
...
async function handler(type) {
    ...
    let list = await axios.get(
      `https://traodoisub.com/api/?fields=${type}&access_token=${config.tds_token}`
    );
		if (!list?.data?.length) {
      console.log(
        "Chạy API Lấy danh sách nhiệm vụ facebook thất bại",
        type,
        list.data
      );
      return false;
    ...
    console.log(
      "Chạy API Lấy danh sách nhiệm vụ facebook, response",
      type,
      list.data
    );
...
async function getCoin(type, id) {
    ...
    let res = await axios.get(
      `https://traodoisub.com/api/coin/?type=${type.toUpperCase()}&id=${id}&access_token=${
        config.tds_token
      }`
    );
    if (!res?.data?.error)
      console.log("Chạy API nhận xu thành công, response", res.data);
	} catch (ex) {
    console.log("err", ex);
...
async function seedingComment(msg) {
  try {
    await visit(`https://mbasic.facebook.com/`);
    let seeding = await findbyCSS(".cl.cm > a:not(:nth-child(3n+1)):first-of-type");
	seeding.click();
	cmt(null, msg);
  } catch (err) {
    console.log("err", err);
